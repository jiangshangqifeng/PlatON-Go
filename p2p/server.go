	...
	for {
		...
		case n := <-srv.removeconsensus:
			...
			if p, ok := peers[n]; ok {
				...
				srv.log.Debug("Remove consensus flag", "peer", n, "consensus", srv.consensus)
				if len(peers) > srv.MaxPeers && !p.rw.is(staticDialedConn|trustedConn) {
					srv.log.Debug("Disconnect non-consensus node", "peer", n, "flags", p.rw.flags, "peers", len(peers), "consensus", srv.consensus)
					p.Disconnect(DiscRequested)
				}
			}
