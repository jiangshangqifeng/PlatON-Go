...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		func Deploy{{.Type}}(auth *bind.TransactOpts, backend bind.ContractBackend {{range .Constructor.Inputs}}, {{.Name}} {{bindtype .Type}}{{end}}) (common.Id, *types.Transaction, *{{.Type}}, error) {
		  parsed, err := abi.JSON(strings.NewReader({{.Type}}ABI))
		  if err != nil {
		    return common.Id{}, nil, nil, err
		  }
	...
	func New{{.Type}}(address common.Id, backend bind.ContractBackend) (*{{.Type}}, error) {
	...
	func New{{.Type}}Caller(address common.Id, caller bind.ContractCaller) (*{{.Type}}Caller, error) {
	...
	func New{{.Type}}Transactor(address common.Id, transactor bind.ContractTransactor) (*{{.Type}}Transactor, error) {
	...
	// New{{.Type}}Filterer creates a new log filterer instance of {{.Type}}, bound to a specific deployed contract.
 	func New{{.Type}}Filterer(address common.Id, filterer bind.ContractFilterer) (*{{.Type}}Filterer, error) {
	...
	func bind{{.Type}}(address common.Id, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	  parsed, err := abi.JSON(strings.NewReader({{.Type}}ABI))
	  if err != nil {
	    return nil, err
	  }
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		public {{.Type}}(Id address, EthereumClient client) throws Exception {
