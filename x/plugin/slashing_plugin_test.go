...
func TestSlashingPlugin_ZeroProduceProcess(t *testing.T) {
	...
	if val, err := rlp.EncodeToBytes(can); nil != err {
		t.Fatal(err)
	} else if err := snapshotdb.Instance().PutBaseDB(staking.CanBaseKeyByID(canAddr), val); nil != err {
		t.Fatal(err)
	}
	if val, err := rlp.EncodeToBytes(can.CandidateMutable); nil != err {
		t.Fatal(err)
	} else if err := snapshotdb.Instance().PutBaseDB(staking.CanMutableKeyByID(canAddr), val); nil != err {
		t.Fatal(err)
	}
